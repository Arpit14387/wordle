...
import raw from "../checkword6.txt";
import word from "../check6.txt";
...
// localStorage.clear("easymodestats")
  if(localStorage.getItem("hardmodestats"))
  {
   
    let data=JSON.parse(localStorage.getItem("hardmodestats"))
    loss=data.loss;
    wins=data.wins;
    total=data.total;
    streak=data.streak;
  ...
  {
    localStorage.setItem("hardmodestats",
    JSON.stringify({
      "total": 0,
      "wins": 0,
      "loss": 0,
    ...
    //   setMessage2("");
     ...
     const getmeaning= async (req,res)=>{
         res= await axios.get(`https://api.dictionaryapi.dev/api/v2/entries/en/${actual}`)
          console.log(res.data[0].meanings[0].definitions[0].definition)
          setHint(res.data[0].meanings[0].definitions[0].definition);
          document.getElementById("msg2").style.display="initial"
      }
    ...
    JSON.stringify({
      ...
      "streak": streak
        ...
        else if(sessionval.length==18 && !(v=="gggggg"))
        {
        getmeaning()
        
        }
        
        else if(!(v=="gggggg") && sessionval.length==24){
      ...
      function closeNav() {
        ...
        return (
          <div className="App" id="body">
           ...
           <div id="notaword"></div>
           <div className="msg2" style={{"display": "none"}} id="msg2">{hint}</div>
            ...
            <div className="hardtemp">
              {
                ...
                arr1.map((x,index)=>{
                  ...
                  var f=sessionval[index+6*(ind)];
                
                  return(
                    <as className="box7" >
                   <span className={` ${f} box3 `} style={{"animationDelay":`${index/2}s`}} id={`${index+6*(ind)}`} ></span>
